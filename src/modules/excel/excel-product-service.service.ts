...
import { ConvertProductServiceDto } from './dto/convert-product-service.dto';
...
@Injectable()
export class ExcelProductServiceService {
  constructor(
  ...
  async importProductServiceExcel(file: Express.Multer.File, res: Response, create_by: string): Promise<void> {
    if (!file) {
  ...
  async filterDataProduct(
    ...
    await Promise.all(
      dataInputConvert.map(async (_) => {
        ...
        const service_charges = this.excelService.formatServiceCharges(_?.service_charges_input);
        ...
        if (descError.length > 0) {
          ...
          productServiceValid.service_charges = service_charges;
  ...
  convertDataProductExcel(
    ...
    return data.map((_) => {
      return {
        ...
        unit_name: _?.__EMPTY_8,
        import_date: _?.__EMPTY_9,
        contract_expire_date: _?.__EMPTY_10,
        producer_name: _?.__EMPTY_11,
        currency_unit_name: _?.__EMPTY_12,
        buying_price: _?.__EMPTY_13,
        service_charges_input: _?.__EMPTY_14,
        buying_fee: {
          price: _?.__EMPTY_15,
          unit_name: _?.__EMPTY_16,
        },
        active_date: _?.__EMPTY_17,
        inactive_date: _?.__EMPTY_18,
        status_input: _?.__EMPTY_19,
        desc: _?.__EMPTY_20,
        row: _?.row,
      };
    });
